...
const Navbar = () => {
    ...
    useEffect(() => {
        if(searchQuery.length > 0){
            navigate('/products'); // Navigate to products page when search query is present
        }
    }, [searchQuery]); // Effect to navigate to products page when search query changes

  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">

            <NavLink to='/' onClick={()=>setOpen(false)} >{/*Whenever mobile menu is open and when we click on the logo it closes menu*/}
        ...
        {open && (
            <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                ...
                {!user ? (
                    ...
                    <button onClick={logout} className="cursor-pointer px-6 py-2 mt-2 bg-primary-dull hover:bg-primary-dull transition text-white rounded-full text-sm">
                    Logout
                </button>
                )}
            </div>)
