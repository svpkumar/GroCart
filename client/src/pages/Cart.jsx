...
const Cart = () => {
    ...
    const {products, currency, navigate, cartItems, removeFromCart, getCartCount, 
        updateCartItems, getcartAmount, axios, user, setCartItems } = useAppContext();
        ...
        try {
            ...
            if(data.success){
                ...
                if(data.addresses.length > 0){
                    ...
                    console.log('Error message0:', data.message);
                }
            }else{
                toast.error(data.message);
                console.log('Error message1:', data.message);
            }
        } catch (error) {
    ...
    const placeOrder = async () =>{
        try {
            if (!user || !user._id) {
                return toast.error("User not logged in. Please sign in first.");
            }
            if(!selectedAddress){
            ...
            if(paymentOption === "COD"){
                ...
                if(data.success){
                    ...
                    toast.error(data.message);
                    console.log('Error message3:', data.message);
                }
            }else{
                ...
                if(data.success){
                    ...
                    toast.error(data.message);
                    console.log('Error message4:', data.message);
                }
            }
        } catch (error) {
            toast.error(error.message);
    ...
    useEffect(() => {
        if(products.length > 0 && cartItems){//call getCart function only when products and cartItems are available
            getCart();
        }
    },[products, cartItems]);
    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row mt-16">
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white 
                font-medium hover:bg-primary-dull transition">
